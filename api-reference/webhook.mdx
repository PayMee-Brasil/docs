...
These notifications track the lifecycle of a Pix Automatic mandate (the recurrence/authorization). They are sent whenever the authorization status changes. The event is identified by the `eventType` field:
...
<Note>
Individual collections generated by a recurrence do **not** emit `PAYMENT_CONFIRMATION`. To track each charge, query the scheduled payments of the authorization (`GET /v1.1/pix-automatic/scheduled-payments`) and rely on `RECURRENCE_CONCLUDED` for the end of the cycle. All dates are in `America/Sao_Paulo` (GMT-3).
</Note>
