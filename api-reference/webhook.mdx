...
<Note>
**Each paid charge produces two webhooks:** a standard [`PAYMENT_CONFIRMATION`](#payment-confirmation-notification) (identical to any Pix payment) **and** a `RECURRENCE_CHARGE_PAID`. Use `RECURRENCE_CHARGE_PAID` to tie the payment to its recurrence and plan: it always carries `idRec`, your `externalId`, `planId` and the `saleToken`/`chargeAmount` of that specific charge. The `saleToken` matches the one in the corresponding `PAYMENT_CONFIRMATION`, so you can join the two. A plan with 20 paid cycles therefore produces 20 of each. You can also reconcile at any time via `GET /v1.1/pix-automatic/scheduled-payments`. All dates are in `America/Sao_Paulo` (GMT-3).
</Note>
...
| `saleToken`           | string | **`RECURRENCE_CHARGE_PAID` only.** Token of the paid charge — matches the `saleToken` of the corresponding `PAYMENT_CONFIRMATION`. |
| `chargeAmount`        | number | **`RECURRENCE_CHARGE_PAID` only.** Amount of that specific paid charge.     |
