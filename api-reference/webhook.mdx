...
These notifications track the lifecycle of a Pix Automatic **mandate** (the recurrence/authorization) — not the individual charges. They are sent whenever the authorization status changes. The event is identified by the `eventType` field:
...
<Note>
**Each paid charge of a recurrence is notified with a standard [`PAYMENT_CONFIRMATION`](#payment-confirmation-notification) webhook** — the same notification as any other Pix payment. So a plan with 20 paid cycles produces 20 `PAYMENT_CONFIRMATION` notifications. Identify each one by its unique `saleToken`; the `referenceCode` carries the recurrence `externalId`, telling you which recurrence the charge belongs to. The `RECURRENCE_*` events above are only for the mandate lifecycle (authorization → conclusion), never for individual charges. You can also reconcile charges at any time via `GET /v1.1/pix-automatic/scheduled-payments`. All dates are in `America/Sao_Paulo` (GMT-3).
</Note>
