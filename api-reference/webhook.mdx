...
The loan status notification reports credit decision changes for BNPL transactions. It uses a PayMee-owned contract so integrations do not depend on a credit provider's payload.

<Warning>
This webhook contract is currently in preview and is published for integration planning. Do not rely on it in production until PayMee confirms enablement for your account.
</Warning>
...
| :---------- | :-------------- |
| `APPROVED`  | The credit proposal was approved and the BNPL contract was completed. |
| `DENIED`    | The credit proposal was declined. |
| `ABANDONED` | The customer did not complete the onboarding flow before expiration. |
...

{
  ...
  "saleToken": "d59b39ce-bffd-3f6d-80c4-c376a242afd1",
  "referenceCode": "order-00000014",
  "status": "APPROVED",
  "date": "2026-08-21 10:48:56"
}
```
...
| Property        | Type   | Description |
| :-------------- | :----- | :---------- |
| `eventType`     | string | Event identifier. Always `LOAN_STATUS_CHANGED`. |
| `saleToken`     | string | PayMee UUID of the BNPL transaction. |
| `referenceCode` | string | Merchant order identifier provided when the loan transaction was created. |
| `status`        | string | Current credit decision: `APPROVED`, `DENIED`, or `ABANDONED`. |
| `date`          | string | Event timestamp in `America/Sao_Paulo` (`yyyy-MM-dd HH:mm:ss`). |

<Note>
Loan approval is distinct from an installment payment. When the associated merchant transaction is financially confirmed, PayMee sends the standard [`PAYMENT_CONFIRMATION`](#payment-confirmation-notification) webhook. Overdue installment and chargeback events are not part of the loan status contract.
</Note>

**Requirements:**
* A valid **Payment Status URL** must be configured in your PayMee merchant panel settings.
* Your endpoint must follow the authentication, acknowledgement, retry, and idempotency requirements described above.
...
| `eventType`               | When it is sent                                                                 |
| :------------------------ | :------------------------------------------------------------------------------ |
| `RECURRENCE_AUTHORIZED`   | The payer approved the mandate; the recurrence is now `APPROVED` and can be charged. |
| `RECURRENCE_REJECTED`     | The payer rejected the authorization request (`REJECTED`).                       |
| `RECURRENCE_EXPIRED`      | The authorization request was not answered in time and expired (`EXPIRED`).      |
| `RECURRENCE_CANCELLED`    | The mandate was cancelled, by either party (`CANCELLED`).                         |
| `RECURRENCE_CONCLUDED`    | The recurrence reached the end of its agreed period (`CONCLUDED`).               |
| `RECURRENCE_CHARGE_SCHEDULED` | A new charge (cycle) was registered at the payer's bank. Carries `chargeDueDate` and `chargeAmount`. |
| `RECURRENCE_CHARGE_PAID`  | A scheduled charge (cycle) of the recurrence was paid. Carries the paid `saleToken` and `chargeAmount`. |
| `RECURRENCE_CHARGE_FAILED` | A charge failed or was rejected (e.g. insufficient funds). Carries `rejectionCode`, `rejectionReason` and `retryScheduled` (whether an automatic retry was scheduled). |

<Note>
Recurring charges are notified **exclusively** through the `RECURRENCE_*` events — they do **not** emit the generic [`PAYMENT_CONFIRMATION`](#payment-confirmation-notification) (only the optional immediate payment of a Journey-3 checkout does, since it carries your own order `referenceCode`). Each charge cycle produces `RECURRENCE_CHARGE_SCHEDULED` when registered, then `RECURRENCE_CHARGE_PAID` or `RECURRENCE_CHARGE_FAILED`. Every event carries `idRec`, your `externalId`, `planId` and, for charge events, the `saleToken` of that specific charge for reconciliation. You can also reconcile at any time via `GET /v1.1/pix-automatic/scheduled-payments`. All dates are in `America/Sao_Paulo` (GMT-3).
</Note>
...
{
  "eventType": "RECURRENCE_AUTHORIZED",
  "idRec": "RR286838922026062203474822291",
  "externalId": "your-order-ref-001",
  "planId": "6a38da8623711b0007b267a6",
  "contractId": "6a38da8623711b0007b267a6",
  "contractDescription": "Monthly subscription",
  "status": "APPROVED",
  "periodicity": "MONTHLY",
  "valueType": "FIXED",
  "amount": 15.00,
  ...
  "payerName": "John Doe",
  "initialDate": "2026-06-25 00:00:00",
  "finalDate": null,
  "nextRecurrenceDate": "2026-07-25 00:00:00",
  "eventDate": "2026-06-22 21:48:34"
}
```
...
```json
{
  "eventType": "RECURRENCE_CHARGE_PAID",
  "idRec": "RR286838922026062203474822291",
  "externalId": "your-order-ref-001",
  "planId": "6a38da8623711b0007b267a6",
  "status": "APPROVED",
  ...
  "payerName": "John Doe",
  "nextRecurrenceDate": "2026-07-25 00:00:00",
  "saleToken": "2ce02094-ab3b-4649-aac8-1a8f122b0ca1",
  "chargeAmount": 15.00,
  "chargeDueDate": "2026-06-25 00:00:00",
  "chargeStatus": "PAID",
  "eventDate": "2026-06-23 16:54:22"
}
```
...
```json
{
  "eventType": "RECURRENCE_CHARGE_FAILED",
  "idRec": "RR286838922026062203474822291",
  "externalId": "your-order-ref-001",
  "planId": "6a38da8623711b0007b267a6",
  "status": "APPROVED",
  ...
  "payerDocument": "12345678909",
  "payerName": "John Doe",
  "saleToken": "bae33c40-df9f-4fd4-ba9a-cbf7cca1a622",
  "chargeAmount": 15.00,
  "chargeDueDate": "2026-07-25 00:00:00",
  "chargeStatus": "REJECTED",
  "rejectionCode": "AM04",
  "rejectionReason": "Saldo insuficiente na conta do pagador",
  "retryScheduled": true,
  "eventDate": "2026-07-25 08:00:12"
}
```
...
| Property              | Type   | Description                                                                 |
| :-------------------- | :----- | :-------------------------------------------------------------------------- |
| `eventType`           | string | The recurrence event (see the table above).                                |
| `idRec`               | string | The acquirer identifier of the recurrence/mandate.                          |
| `externalId`          | string | Your unique reference provided when the recurrence was created.             |
| `planId`              | string | Identifier of the plan the recurrence belongs to.                          |
| `contractId`          | string | Identifier of the contract (mirrors `planId`).                             |
| `contractDescription` | string | Human-readable description of the plan/contract.                          |
| `status`              | string | The new status of the recurrence (e.g. `APPROVED`, `REJECTED`, `EXPIRED`, `CANCELLED`, `CONCLUDED`). |
| `periodicity`         | string | Charge frequency (`WEEKLY`, `MONTHLY`, `QUARTERLY`, `SEMIANNUAL`, `ANNUALLY`). |
| `valueType`           | string | `FIXED` for a fixed amount or `VARIABLE` for a per-cycle amount.            |
| `amount`              | number | The recurrence amount.                                                     |
| `payerDocument`       | string | The payer's CPF/CNPJ.                                                       |
| `payerName`           | string | The payer's name.                                                          |
| `initialDate`         | string | Start date of the recurrence (`yyyy-MM-dd HH:mm:ss`).                       |
| `finalDate`           | string | End date of the recurrence, or `null` if open-ended.                       |
| `nextRecurrenceDate`  | string | Date of the next scheduled charge (`yyyy-MM-dd HH:mm:ss`).                  |
| `saleToken`           | string | **Charge events only.** Token of that specific charge (use it for reconciliation). |
| `chargeAmount`        | number | **Charge events only.** Amount of that specific charge.                    |
| `chargeDueDate`       | string | **Charge events only.** Due date of that charge (`yyyy-MM-dd HH:mm:ss`).   |
| `chargeStatus`        | string | **Charge events only.** Status of the charge (`SCHEDULED`, `PAID`, `FAILED`, `REJECTED`, `CANCELLED`). |
| `rejectionCode`       | string | **`RECURRENCE_CHARGE_FAILED` only.** Bacen reason code (e.g. `AM04` insufficient funds, `AC06` blocked account). |
| `rejectionReason`     | string | **`RECURRENCE_CHARGE_FAILED` only.** Human-readable description of the failure. |
| `retryScheduled`      | boolean | **`RECURRENCE_CHARGE_FAILED` only.** `true` when an automatic retry was scheduled (mandates with `ACCEPT_3R_7D`). |
| `eventDate`           | string | Timestamp the notification was generated (`yyyy-MM-dd HH:mm:ss`).          |
